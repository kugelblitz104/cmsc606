    ...
    for ftuple in f[1:]:
        ...
        match op:
            ...
            case "/":
                vars.append(vars[u] / vars[v])
            case "S":
                vars.append(np.square(vars[u]))
            case "E":
                vars.append(np.exp(vars[u]))
            case "L":
                vars.append(np.log(vars[u]))
            case "X":
                vars.append(float(u))
            case "C":
                vars.append(float(u))
    ...
    for ftuple in f:
        ...
        match op:
            ...
            case "-":
                dotted.append(dotted[u] - dotted[v])
            case "*":
                dotted.append((dotted[u] * vars[v]) + (vars[u] * dotted[v]))
            case "/":
                dotted.append(
                    ((dotted[u] * vars[v]) - (vars[u] * dotted[v]))
                    / (np.square(vars[v]))
                )
            case "S":
                dotted.append((2.0 * vars[u]) * (dotted[u]))
            case "E":
                dotted.append((np.exp(vars[u])) * (dotted[u]))
            case "L":
                dotted.append(dotted[u] / vars[u])
            case "X":
                dotted.append(1.0)
            case "C":
                dotted.append(0.0)
